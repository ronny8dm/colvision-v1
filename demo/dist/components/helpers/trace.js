 ...
 *    }
 */
const getLineBreakpoints = (rleMask, height) => {
    const breakpoints = [];
    const currentLine = { line: -1, points: [] };
 ...
 */
export const generatePolygonSegments = (rleMask, height) => {
    ...
    const addToPolySegments = (p1, p2) => {
        const p1Str = `${p1.x} ${p1.y}`, p2Str = `${p2.x} ${p2.y}`;
        if (!polySegments.has(p1Str))
            polySegments.set(p1Str, new Set());
        polySegments.get(p1Str).add(p2Str);
        if (!polySegments.has(p2Str))
            polySegments.set(p2Str, new Set());
    ...
    const closeHorizontalSegment = (y, x2) => {
    ...
    const closePreviousLine = (prevLine, prevPoints) => {
    ...
    for (const { line, points } of breakpoints) {
        ...
        while (lastLineIndex < lastPoints.length || newLineIndex < points.length) {
            ...
            let x2, y2;
            if (lastLineIndex === lastPoints.length ||
    ...
    const sortedSegments = new Map([...polySegments].sort((a, b) => {
 ...
 */
export const convertSegmentsToSVG = (polySegments) => {
